/*
  ...
  1. Performance Improvements
    ...
    - Replace direct jwt() calls with (select jwt()) to prevent re-evaluation
    - Consolidate duplicate permissive policies to improve query performance

  2. Policy Changes
    - Remove redundant auth_* policies that overlap with admin policies
    - Update admin policies to use optimized auth function calls
    - Maintain existing security rules while improving performance
*/
...
CREATE POLICY "Admins can insert users"
  ...
  FOR INSERT
  TO authenticated
  WITH CHECK ((SELECT (jwt() ->> 'role'::text)) = 'admin'::text);
...
CREATE POLICY "Admins can update users"
  ...
  FOR UPDATE
  TO authenticated
  USING ((SELECT (jwt() ->> 'role'::text)) = 'admin'::text)
  WITH CHECK ((SELECT (jwt() ->> 'role'::text)) = 'admin'::text);
...
CREATE POLICY "Users can read themselves and admins can read all"
  ...
  FOR SELECT
  TO authenticated
  USING ((SELECT (jwt() ->> 'role'::text)) = 'admin'::text OR (SELECT uid()) = id);